...
void CourseLoader::LoadCourseFileFromManager(const std::string& directoryPath, const std::string& filename, CourseImportData& course) {
	//ファイルオープン
	std::string filepath = directoryPath + "/" + filename;
	FILE* fp;
...
void CourseLoader::CreateCoursePolygons(const CourseImportData& data, std::vector<CoursePolygon>& course) {
	CoursePolygon cData;
